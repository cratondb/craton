...
//!
//! Tests under `tests/` are each compiled as a separate binary, so
//! anything here is duplicated into every test that imports it. Keep
//! it small and focused on the cross-test plumbing (port reservation,
//! binary discovery, minimal HTTP client) — scenario-specific logic
//! lives in the test files themselves.
...
/// supervisor's convention in `crates/kimberlite-cluster/src/node.rs`.
...
pub fn poll_http(
    ...
    while Instant::now() < stop {
        match http_get(addr, path) {
            ...
            Err(_) => {} // Connection refused / reset — retry.
        }
        std::thread::sleep(Duration::from_millis(200));
    }
    match last {
