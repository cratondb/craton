...
Kimberlite is a verifiable database for healthcare. This audit examined the full source code across 33 workspace crates, verified formal verification claims against implementations, and tested compliance mechanisms against HIPAA (Privacy + Security + Breach Notification Rules) with GDPR / SOC 2 overlays. *(Note: written pre-Q1-2026 healthcare pivot; legacy SOX/PCI-DSS scope is recorded but no longer in product scope.)*
